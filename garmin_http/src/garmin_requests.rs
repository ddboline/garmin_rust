...
impl FitbitStatisticsPlotRequest {
    ...
    pub async fn get_plots(
        ...
        get_fitbit_statistics_plots(&stats, self.request.offset).map_err(Into::into)
    }
}

fn get_fitbit_statistics_plots(
    stats: &[FitbitStatisticsSummary],
    offset: Option<usize>,
) -> Result<HashMap<StackString, StackString>, Error> {
    ...
    let js_str = serde_json::to_string(&min_heartrate).unwrap_or_else(|_| String::new());
    let params = hashmap! {
    ...
    let max_heartrate: Vec<_> = stats
        ...
        .collect();
    let js_str = serde_json::to_string(&max_heartrate).unwrap_or_else(|_| String::new());
    let params = hashmap! {
    ...
    let mean_heartrate: Vec<_> = stats
        ...
        .collect();
    let js_str = serde_json::to_string(&mean_heartrate).unwrap_or_else(|_| String::new());
    let params = hashmap! {
    ...
    let median_heartrate: Vec<_> = stats
        ...
        .collect();
    let js_str = serde_json::to_string(&median_heartrate).unwrap_or_else(|_| String::new());
    let params = hashmap! {
...
impl ScaleMeasurementPlotRequest {
    ...
    pub async fn get_plots(
        ...
        .await?;
        get_scale_measurement_plots(&measurements, self.request.offset, config).map_err(Into::into)
    }
}

fn get_scale_measurement_plots(
    ...
    let js_str = serde_json::to_string(&mass).unwrap_or_else(|_| String::new());
    let params = hashmap! {
        "EXAMPLETITLE" => "Weight",
        "XAXIS" => "Date",
        "YAXIS" => "Weight [lbs]",
    ...
    let js_str = serde_json::to_string(&fat).unwrap_or_else(|_| String::new());
    let params = hashmap! {
        "EXAMPLETITLE" => "Fat %",
        "XAXIS" => "Date",
        "YAXIS" => "Fat %",
    ...
    let js_str = serde_json::to_string(&water).unwrap_or_else(|_| String::new());
    let params = hashmap! {
        "EXAMPLETITLE" => "Water %",
        "XAXIS" => "Date",
        "YAXIS" => "Water %",
    ...
    let js_str = serde_json::to_string(&muscle).unwrap_or_else(|_| String::new());
    let params = hashmap! {
        "EXAMPLETITLE" => "Muscle %",
        "XAXIS" => "Date",
        "YAXIS" => "Muscle %",
    ...
    let js_str = serde_json::to_string(&bone).unwrap_or_else(|_| String::new());
    let params = hashmap! {
        "EXAMPLETITLE" => "Bone %",
        "XAXIS" => "Date",
        "YAXIS" => "Bone %",
