...
fn test_garmin_config_new() {
    ...
    let default_gps_dir = format!("{}/.garmin_cache/run/gps_tracks", home_dir).into();

    let gc = garmin_config::GarminConfig::new();

    assert_eq!(gc.port, 8000);
    assert_eq!(gc.pgurl.as_str(), "");
...
fn test_garmin_config_get_config() {
    ...
    assert_eq!(gc.maps_api_key.as_str(), "TESTKEY");
    assert_eq!(
        gc.pgurl.as_str(),
        "postgresql://test:test@localhost:5432/garmin_summary_test"
    );
    assert_eq!(gc.gps_dir.as_str(), "/tmp/gps_dir");
}
